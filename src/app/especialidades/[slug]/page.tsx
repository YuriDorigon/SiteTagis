...
export async function generateStaticParams() {
  ...
  return specialties.map((s) => ({ slug: slugify(s.name) }));
}
...
export default async function EspecialidadePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [specialties, doctors] = await Promise.all([getSpecialties(), getDoctors()]);
  const specialty = specialties.find((s) => slugify(s.name) === slug);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: specialty.name, item: `${siteUrl}/especialidades/${slug}` },
    ],
  };

  const waMessage = encodeURIComponent(`Olá! Gostaria de agendar uma consulta de ${specialty.name}.`);

  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(specialtyLd) }} />
      ...
      <div className="container mx-auto px-4 md:px-6 py-12 md:py-16 max-w-4xl">
        ...
        <nav className="flex items-center gap-2 text-xs text-foreground/40 mb-10" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <Link href="/especialidades" className="hover:text-primary transition-colors">Especialidades</Link>
          <span>/</span>
        ...
        <div className="flex items-start gap-6 mb-10">
          <div className="p-4 bg-primary/8 rounded-2xl flex-shrink-0">
            <LucideIconRenderer name={specialty.iconName} className="h-10 w-10 text-primary" />
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-primary font-headline mb-2">{specialty.name}</h1>
            <p className="text-foreground/60 leading-relaxed">{specialty.description}</p>
          </div>
        </div>
        ...
        {relatedDoctors.length > 0 && (
          <div className="mb-12">
            <h2 className="text-lg font-semibold text-primary font-headline mb-6 flex items-center gap-2">
              ...
              Médicos que atendem {specialty.name}
            </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center border-t border-border pt-8">
          ...
          <Link href="/especialidades" className="inline-flex items-center gap-1.5 text-sm text-foreground/50 hover:text-primary transition-colors">
            <ArrowLeft className="h-4 w-4" /> Ver todas as especialidades
          </Link>
        </div>
      </div>
    </>
